...
var (
	...
	}, []string{"delta", "release", "compare_release", "platform", "backend", "upgrade_type", "master_nodes_updated", "network", "topology", "architecture", "feature_set", "os", "releaseStatus"})
...
func RefreshMetricsDB(ctx context.Context, dbc *db.DB, bqc *bqclient.Client, crProvider dataprovider.DataProvider, reportEnd time.Time, cacheOptions cache.RequestOptions, views []crview.View) error {
	...
	if crProvider != nil {
		refreshComponentReadinessMetrics(ctx, crProvider, dbc, cacheOptions, views, releases)
	}

	// BigQuery-only metrics
	if bqc != nil {
...
func updateComponentReadinessMetricsForView(ctx context.Context, provider dataprovider.DataProvider, dbc *db.DB, cacheOptions cache.RequestOptions, view crview.View, releases []v1.Release) error {
	...
	report, errs := componentreadiness.GetComponentReport(ctx, provider, dbc, reportOpts, "")
	if len(errs) > 0 {
...
func refreshDisruptionMetrics(client *bqclient.Client, releases []v1.Release) error {
	...
	for _, row := range disruptionReport.Rows {
		...
		disruptionVsPrevGAMetric.WithLabelValues("P50",
			...
			row.MasterNodesUpdated, row.Network, row.Topology, row.Architecture, row.FeatureSet, row.OS, releaseStatus).Set(float64(row.P50))
		disruptionVsPrevGAMetric.WithLabelValues("P75",
			row.Release, row.CompareRelease, row.Platform, row.BackendName, row.UpgradeType,
			row.MasterNodesUpdated, row.Network, row.Topology, row.Architecture, row.FeatureSet, row.OS, releaseStatus).Set(float64(row.P75))
		disruptionVsPrevGAMetric.WithLabelValues("P95",
			row.Release, row.CompareRelease, row.Platform, row.BackendName, row.UpgradeType,
			row.MasterNodesUpdated, row.Network, row.Topology, row.Architecture, row.FeatureSet, row.OS, releaseStatus).Set(float64(row.P95))
		disruptionVsPrevGAMetric.WithLabelValues("PercentageAboveZero",
			row.Release, row.CompareRelease, row.Platform, row.BackendName, row.UpgradeType,
			row.MasterNodesUpdated, row.Network, row.Topology, row.Architecture, row.FeatureSet, row.OS, releaseStatus).Set(float64(row.PercentageAboveZeroDelta))
		disruptionVsPrevGARelevanceMetric.WithLabelValues(
			row.Release, row.CompareRelease, row.Platform, row.BackendName, row.UpgradeType,
			row.MasterNodesUpdated, row.Network, row.Topology, row.Architecture, row.FeatureSet, row.OS, releaseStatus).Set(float64(row.Relevance))
	}
